...
bool Compare(const RouteModel::Node *a, const RouteModel::Node *b) {
    ...
    float f2 = b->g_value + b->h_value;  // f2 = + h2
    return f1 > f2;
}
...
//     distance += start_node->distance(*current_node);
...
void RoutePlanner::AStarSearch() {
    ...
    while (open_list.size() != 0) {
        if (current_node == end_node) {
            std::cout << "made to the end" << "\n";
            m_Model.path = ConstructFinalPath(current_node);
            break;
        } else {
            std::cout << "add one more node " << open_list.size() << "\n";
...
// cd CppND-Route-Planning-Project/build/ && make && ./test